...
@parser::members {
	...
	int countNewLineOuccrences(const std::string& code) {
		int count = 0;
		for (char c : code) {
			if (c == '\n') {
				count++;
	...
	void setScopeAllowed(){
		isScopeAllowed = true;
	}
	void clearScopeAllowed() {
	...
	Identifier evaluateIncDec(Identifier id, std::string line) {
		...
		changeLabelFromTempAtLine(id.printedNextLabel, label, id.nextLabelLineCount);
...
func_definition :
    type_specifier ID{writeIntoTempFile($ID->getText() + " PROC\n");enterScope();} LPAREN parameter_list RPAREN compound_statement {
		exitScope();
		writeIntoTempFile($ID->getText() + " ENDP\n\n");
	}
    | type_specifier ID{writeIntoTempFile($ID->getText() + " PROC\n");} LPAREN RPAREN {initializeMainFunction($ID->getText());enterScope();} compound_statement {
		exitScope();
		if ($ID->getText() == "main") {
			exitMainFunction();
		}
...
statement :
    ...
    | IF LPAREN e = expression{generateCodeAfterIfExpression($e.id);} RPAREN s = statement {
		generateCodeAfterIfStatement($e.id);
		generateCodeAfterElseStatement($e.id);
	}
    | IF LPAREN e = expression{generateCodeAfterIfExpression($e.id);} RPAREN statement{generateCodeAfterIfStatement($e.id);} ELSE statement {
		generateCodeAfterElseStatement($e.id);
	}
...
expression returns [Identifier id] :
    ...
    | v = variable ASSIGNOP le = logic_expression {
		if($le.id.printedTrueLabel == "EMPTY" && $le.id.printedFalseLabel == "EMPTY") {
...
logic_expression returns [Identifier id] :
    ...
    | re1 = rel_expression LOGICOP re2 = rel_expression {
		$re2.id = generateRelationalCodeForSimpleExpression($re2.id, to_string($LOGICOP->getLine()));
		$re1.id = generateRelationalCodeForSimpleExpression($re1.id, to_string($LOGICOP->getLine()));
		$id = generateLogicalCode($re2.id, $re1.id, to_string($LOGICOP->getLine()), $LOGICOP->getText());
    }
;
